...
function ColorListing({
  ...
  const columns = [
    ...
    {
      title: (
        <div className="dark:text-white text-center text-sm">ColorCode</div>
      ),
      dataIndex: "color_code",
      render: (t) => (
        <div className="flex justify-center items-center gap-4 dark:text-white text-sm">
          {t}
          <span
            className="flex h-6 w-6 rounded-full border border-black dark:border-white"
            style={{ background: t }}
          ></span>
        </div>
      ),
      sorter: {
        compare: (a, b) => a.color_code - b.color_code,
      },
    },
    {
      title: (
    ...
    {
      title: <div className="dark:text-white text-center text-sm">Action</div>,
      dataIndex: "id",
      render: (t) => (
  ...
  const HandleDelete = (id) => {
    ...
    fetch(`${Base_Url}customizeSofa/deleteColor/${id}`, requestOptions)
      .then((response) => response.json())
      .then((result) => {
        GetAllData();
        if (result.status === 200) {
          toast.success("Foam Deleted Successfully", {
            duration: 7000,
            position: "top-center",
            reverseOrder: false,
          });
      ...
      <div className="mt-[12px] drop-shadow-lg w-[99%] my-2">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search By Title, ColorCode, CreatedAt"
            value={Search}
        ...
        <Table
          ...
          dataSource={itemsWithSerialNumbers.filter((el) => {
            return (
              ...
              el.color_code.toLowerCase().includes(Search) ||
