...
function CustomSofaCategoryListing({
  Data,
  GetAllData,
  setComponentLoader,
  handleUpdateUser,
  ...
  const HandleDelete = (id) => {
    ...
    fetch(
      `${Base_Url}customizeSofa/deleteCustomSofaCategory/${id}`,
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => {
        if (result.status === 200) {
          toast.success("CustomSofaCategory Deleted Successfully", {
            duration: 7000,
            position: "top-center",
            reverseOrder: false,
          });
...
export default CustomSofaCategoryListing;
