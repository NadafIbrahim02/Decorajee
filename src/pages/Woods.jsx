...
import WoodListing from "../components/WoodListing";
function Woods() {
  ...
  const handleUploadFile = () => {
    ...
    fetch(`${Base_Url}customizeSofa/createWoodImage`, requestOptions)
      ...
      .then((result) => {
        if (result.status === 200) {
          ...
          setApiFormData({ ...ApiFormData, ["image"]: result.data.Wood_image });
          setImg("");
    ...
    if (ApiFormData.title) {
      if (ApiFormData.price) {
        if (ApiFormData.image) {
          ...
          await fetch(`${Base_Url}customizeSofa/createWood`, requestOptions)
            .then((response) => response.json())
            .then((result) => {
              setApiFormData({
                ...ApiFormData,
  ...
  const GetAllData = () => {
    ...
    fetch(`${Base_Url}customizeSofa/getAllWoods`, requestOptions)
      .then((response) => response.json())
      .then((result) => {
  ...
  const HandleOnUpdate = async (e) => {
    ...
    await fetch(
      `${Base_Url}customizeSofa/updateWood/${SingleArray[0].id}`,
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => {
          ...
          <form className="my-4" onSubmit={HandleOnUpdate}>
            ...
            <div class="relative z-0 w-full mb-6 group">
              <input
                ...
                class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                placeholder=" "
                value={ApiFormData.price}
                onChange={(e) =>
                  setApiFormData({ ...ApiFormData, ["price"]: e.target.value })
                }
              />
            </div>
          ...
          <form className="my-4" onSubmit={HandleOnSubmit}>
            ...
            <div class="relative z-0 w-full mb-6 group flex ">
              ...
              {Img ? (
                <span
                  onClick={handleUploadFile}
                  className="py-2 bg-black cursor-pointer rounded-r-xl text-white px-4 flex justify-center items-center"
                >
                  {ButtonLoading ? <Spin indicator={antIcon} /> : "Upload"}
                </span>
              ) : null}
