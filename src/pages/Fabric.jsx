...
function Fabric() {
  ...
  const handleUploadFile = () => {
    ...
    fetch(`${Base_Url}customizeSofa/createFabricImage`, requestOptions)
      ...
      .then((result) => {
        if (result.status === 200) {
          ...
          setApiFormData({
            ...ApiFormData,
            ["image"]: result.data.Fabric_image,
          });
          setImg("");
    ...
    if (ApiFormData.title) {
      if (ApiFormData.price) {
        if (ApiFormData.image) {
          ...
          await fetch(`${Base_Url}customizeSofa/createFabric`, requestOptions)
            .then((response) => response.json())
            .then((result) => {
              setApiFormData({
                ...ApiFormData,
  ...
  const HandleOnUpdate = async (e) => {
    ...
    await fetch(
      `${Base_Url}customizeSofa/updateFabric/${SingleArray[0].id}`,
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => {
