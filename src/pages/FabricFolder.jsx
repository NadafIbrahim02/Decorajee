...
function FabricFolder() {
  ...
  const handleUploadFile = () => {
    ...
    fetch(`${Base_Url}customizeSofa/createFabricFolderImage`, requestOptions)
  ...
  const HandleOnSubmit = async (e) => {
    ...
    if (ApiFormData.title) {
      if (ApiFormData.image) {
        ...
        await fetch(
          `${Base_Url}customizeSofa/createFabricFolder`,
          requestOptions
        )
          .then((response) => response.json())
          .then((result) => {
            ...
            if (result.status === 200) {
              GetAllData();
              setLoading(false);
              toast.success("FabricFolder Created Successfully", {
                duration: 7000, // 7 seconds in milliseconds
                position: "top-center",
                reverseOrder: false,
              });
  ...
  const GetAllData = () => {
    ...
    fetch(`${Base_Url}customizeSofa/getFolderByFabricId/${id}`, requestOptions)
      .then((response) => response.json())
      .then((result) => {
  ...
  useEffect(() => {
    ...
    setApiFormData({ ...ApiFormData, ["fabric_id"]: id });
    GetAllData();
  }, []);
  ...
  const HandleOnUpdate = async (e) => {
    ...
    await fetch(
      `${Base_Url}customizeSofa/updateFabricFolder/${SingleArray[0].id}`,
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => {
