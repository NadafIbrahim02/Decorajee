...
function Foam() {
  ...
  const handleUploadFile = () => {
    ...
    fetch(`${Base_Url}customizeSofa/createFoamImage`, requestOptions)
      ...
      .then((result) => {
        if (result.status === 200) {
          ...
          setApiFormData({ ...ApiFormData, ["image"]: result.data.Foam_image });
          setImg("");
    ...
    if (ApiFormData.title) {
      if (ApiFormData.price) {
        if (ApiFormData.image) {
          ...
          await fetch(`${Base_Url}customizeSofa/createFoam`, requestOptions)
            .then((response) => response.json())
            .then((result) => {
              setApiFormData({
                ...ApiFormData,
              ...
              if (result.status === 200) {
                GetAllData();
                setLoading(false);
                toast.success("Foam Created Successfully", {
                  duration: 7000, // 7 seconds in milliseconds
                  position: "top-center",
                  reverseOrder: false,
                });
  ...
  const GetAllData = () => {
    ...
    fetch(`${Base_Url}customizeSofa/getAllFoams`, requestOptions)
      .then((response) => response.json())
      .then((result) => {
  ...
  const HandleOnUpdate = async (e) => {
    ...
    await fetch(
      `${Base_Url}customizeSofa/updateFoam/${SingleArray[0].id}`,
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => {
        ...
        if (result.status === 200) {
          toast.success("Foam Updated Successfully", {
            duration: 7000,
            position: "top-center",
            reverseOrder: false,
          });
