...
import FabricFolderListing from "../components/FabricFolderListing";
import FabricFolderShadeListing from "../components/FabricFolderShadeListing";
function FabricFolderShade() {
  const { id } = useParams();
  ...
  const handleUploadFile = () => {
    ...
    fetch(
      `${Base_Url}customizeSofa/createFabricFolderShadeImage`,
      requestOptions
    )
      ...
      .then((result) => {
        if (result.status === 200) {
          ...
          setApiFormData({
            ...ApiFormData,
            ["image"]: result.data.FabricFolder_image,
          });
          setImg("");
  ...
  const HandleOnSubmit = async (e) => {
    ...
    if (ApiFormData.title) {
      if (ApiFormData.image) {
        ...
        await fetch(
          `${Base_Url}customizeSofa/createFabricFolderShades`,
          requestOptions
        )
          .then((response) => response.json())
          .then((result) => {
            ...
            if (result.status === 200) {
              GetAllData();
              setLoading(false);
              toast.success("FabricFolderShade Created Successfully", {
                duration: 7000, // 7 seconds in milliseconds
                position: "top-center",
                reverseOrder: false,
              });
  ...
  const GetAllData = () => {
    ...
    fetch(
      `${Base_Url}customizeSofa/getFabricFolderShadeById/${id}`,
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => {
  ...
  useEffect(() => {
    ...
    setApiFormData({ ...ApiFormData, ["fabric_folder_id"]: id });
    GetAllData();
  }, []);
  ...
  const HandleOnUpdate = async (e) => {
    ...
    await fetch(
      `${Base_Url}customizeSofa/updateFabricFolderShade/${SingleArray[0].id}`,
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => {
        ...
        if (result.status === 200) {
          toast.success("FabricFolder Updated Successfully", {
            duration: 7000,
            position: "top-center",
            reverseOrder: false,
          });
        ...
        {ShowUpdatePage ? (
          <form className="my-4" onSubmit={HandleOnUpdate}>
            ...
            <div class="relative z-0 w-full mb-6 group flex ">
              <img
                src={Base_Url + ApiFormData.image}
                alt=""
...
export default FabricFolderShade;
